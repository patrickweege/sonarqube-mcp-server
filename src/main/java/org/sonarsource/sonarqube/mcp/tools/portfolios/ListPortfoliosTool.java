...
public class ListPortfoliosTool extends Tool {
  ...
  public Result execute(Arguments arguments) {
    try {
      ...
      var response = serverApi.portfoliosApi().list(enterpriseId, query, favorite, draft, pageIndex, pageSize);
  ...
  private static String formatResponse(ListResponse response) {
    if (response.components() != null && !response.components().isEmpty()) {
      return formatServerPortfolios(response.components(), response.paging());
    } else if (response.portfolios() != null && !response.portfolios().isEmpty()) {
      return formatCloudPortfolios(response.portfolios(), response.page());
    } else {
      return "No portfolios were found.";
    }
  }

  private static String formatServerPortfolios(List<ListResponse.Component> components, @Nullable ListResponse.Paging paging) {
    ...
    if (paging != null) {
      builder.append("\nThis response is paginated and this is the page ")
        .append(paging.pageIndex()).append(" out of ")
        .append((int) Math.ceil((double) paging.total() / paging.pageSize()))
        .append(" total pages. There is a maximum of ")
        .append(paging.pageSize()).append(" portfolios per page.");
    }

    return builder.toString().trim();
  }

  private static String formatCloudPortfolios(List<ListResponse.Portfolio> portfolios, @Nullable ListResponse.Page page) {
    var builder = new StringBuilder("Available Portfolios:\n\n");
    ...
    addPaginationInfo(builder, page);

    return builder.toString().trim();
  }

  private static void formatPortfolioDetails(StringBuilder builder, ListResponse.Portfolio portfolio) {
  ...
  private static void addPaginationInfo(StringBuilder builder, @Nullable ListResponse.Page page) {
    if (page != null) {
      builder.append("\nThis response is paginated and this is the page ")
