...
class McpLoggerTest {
  ...
  void it_should_send_an_rcp_log_to_the_client() {
    ...
    verify(mockServer).loggingNotification(new McpSchema.LoggingMessageNotification(McpSchema.LoggingLevel.DEBUG, "sonarqube-mcp-server", "Message"));
    verify(mockServer).loggingNotification(new McpSchema.LoggingMessageNotification(McpSchema.LoggingLevel.DEBUG, "sonarqube-mcp-server", "stack\ntrace"));
  }
  ...
  void it_should_send_an_error_log_to_the_client() {
    ...
    assertThat(notifications.getFirst()).isEqualTo(new McpSchema.LoggingMessageNotification(McpSchema.LoggingLevel.ERROR, "sonarqube-mcp-server", "Message"));
    assertThat(notifications.get(1).level()).isEqualTo(McpSchema.LoggingLevel.ERROR);
    assertThat(notifications.get(1).logger()).isEqualTo("sonarqube-mcp-server");
